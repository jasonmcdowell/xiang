...
- Both Playground tabs include **Blast!**, which sends tiles to randomized in-board positions, and **Shuffle**, which moves tiles through short lifted arcs to shuffled grid spaces with synthesized tile click-clacks. The selected arrangement can be kept active so each new tile triggers it after the current drag ends. A tile brought forward by a whole-tile drag stays on top of overlapping tiles. Successful tears use a sharper, rising bubble-pop sound.
- A second **Discovery Run** tab adds a survival game alongside the unchanged free-form Playground. Start with one random character from Playground starters, HSK 1 Simplified, or HSK 1 Traditional; add one tile every 10 seconds; and end when the selected 3×3, 4×4, 5×5, or 6×6 cell board fills. The board is a square Go-inspired grid with tiles centered in cells, sized to fit and snapped to the grid. Score one point per delivered tile and one per unique character outside the draw collection that appears on the board during the run, including children revealed by decomposition even if they are later recombined. The arrival clock pauses when another Playground tab is active.
- A separate tile-writing selector switches character data between Simplified and Traditional in both main games and both Playground tabs. Preserve the live board and rebuild matching composition/decomposition indices; keep this independent of interface language. Variant mappings are generated from OpenCC, and Playground switches only when the destination outline exists and any existing physical recipe can be retained.
