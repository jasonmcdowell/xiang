...
Open `/playground` or use **Playground** in the game header. Choose 想, 相, 明, 休, or 好. Drag a mapped child away from the character; it stretches first, then tears free only when the same-size tile faces have at least 12 pixels of clearance from each other and every existing tile. You can keep decomposing a free child, such as 想 → 相 + 心 → 木 + 目 + 心, while the other pieces stay in the scene. Pull compatible free pieces into the gentle magnetic field, then guide them into their original relative positions to snap them together. The field can catch pieces from a full-tile separation and gently deforms a held piece before it reaches the snap distance; reversed layouts do not attract. On phones, the taller board gives two separated tiles room to stay visible during a vertical pull. Only this small, reviewed recipe set is available in the lab.
