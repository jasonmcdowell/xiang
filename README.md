...
The side panel keeps the focused character's pinyin and English definition visible; clicking a tile or ink changes focus, and a successful tear follows the freed component. A successful tear also plays a short synthesized pop. Double-tap or double-click a tile to unfold exactly one supported decomposition step. **Tile repulsion** is on by default and can be turned off to compare the subtle short-range nudge between movable tiles.
