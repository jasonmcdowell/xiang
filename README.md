...
The current build contains 8,294 decompositions and 16,092 ordered pair keys. The command reports counts, top components, and checks 想 ↔ 相 + 心. `npm test` checks the complete generated dataset, not just the example. The main game can query this full normalized index; the physics playground uses a smaller reviewed set because each recipe also needs complete stroke paths and stroke-to-component mappings.
