...
- `src/lib/discoveryRun.ts` — Discovery Run's independent timer, score, discovery history, and capacity rules.
- `scripts/test-discovery-run.mjs` — browser check for Discovery Run's game loop and cell grid.
...
The playground has two tabs. **Playground** remains the open-ended physics lab. **Discovery Run** starts with one random character from the Playground starter set or HSK 1 Simplified/Traditional, then adds a character from that set every 10 seconds. Choose a 3×3, 4×4, 5×5, or 6×6 square cell board; all tiles use one size fitted to those cells and snap to their centers. The run ends when every cell is occupied. Score one point per delivered tile and one per unique character outside the selected draw set that appears on the board, even if it is later recombined. Switching to Playground pauses the clock; returning resumes the same run. Discovery Run state lasts for the current page visit.
...
Open `/playground` or use **Playground** in the game header. The board starts with five decomposable characters: 想, 相, 明, 休, 好. Choose **Five starters** to reset to that board, use the quick picks, or enter any character with a local Make Me a Hanzi outline. **Explore** puts the character on a one-tile board; **Add** places it in the nearest open, non-overlapping spot while keeping the current board. Reset restores a custom board to the characters and positions it had when its first character was added. The catalog contains 9,565 drawable characters and 8,182 complete pairwise physical recipes from the current source data. A character with an outline but no complete supported mapping can still be explored and moved, but cannot be torn apart; dragging one of its strokes moves its tile and ink together. Unreviewed three-or-more-child structures are not flattened into physical recipes. 林 and 森 remain quick picks; 森 unfolds into 木 + 林, and 林 unfolds into 木 + 木, so three 木 pieces can be recombined as 木 + 木 → 林, then 林 + 木 → 森. The physics controls and instructions sit beside the expanded gameboard on desktop and below it on phones. Drag a mapped child away from the character; the remaining strokes stay in place, and the split happens as soon as the two child faces clear each other and fit on the board. The split does not wait for unrelated tiles to move out of the way, so a new face may temporarily overlap another tile. Its tile follows the held strokes until release, then the strokes settle back onto it. You can keep decomposing a free child, such as 想 → 相 + 心 → 木 + 目 + 心, while the other pieces stay in the scene. Separated pieces do not attract from a distance. To recombine them, overlap compatible tiles; for a detachable mapped piece, holding its ink over the compatible tile can also guide the strokes into relative layout. If a character has no supported tear recipe, dragging its strokes moves the whole tile and ink together. The tug grows with contact and strengthens as the strokes approach their target; merely touching tile edges does not engage it, and reversed layouts do not attract. The taller board gives separated tiles room to stay visible during a pull.

Use **Surface style** to compare Flat, Raised, Draped, and Silk rendering. Raised, Draped, and Silk place the character ink on a mahjong-inspired tile; every tile face remains the same 156 × 156 CSS pixels. Raised ink lifts and casts a soft shadow as it is pulled. Draped ink bends over its tile edge toward tabletop height. Silk doubles the visible sidewall depth and lets pulled ink sink to the table with a soft ground shadow; when the strokes cross another tile, they rise back onto its top. Silk uses the same projected stroke geometry for hit-testing, and gives its lattice a softer response. During a component pull, the source tile stays in place even in Weighted mode. If the component tears free, its new tile follows the held drag at the position where it separated until release. Once released, unheld ink gently settles toward its own tile's center and orientation unless it is being magnetically aligned for a valid composition; it does not recombine by itself. Start on an unprinted area of the face to grip the whole tile: Weighted moves the tile and ink together with resistance, while Fixed preserves the tile's anchor. These are fixed-camera 2.5D drawing experiments on the existing 2D physics, not 3D simulations.
...
Run `npm test` for simulation/rules checks, `XIANG_TEST_URL=http://127.0.0.1:3001 npm run test:playground` against a running server for physical Playground interactions, and `XIANG_TEST_URL=http://127.0.0.1:3001 npm run test:discovery` for the Discovery Run browser flow. The latter checks grid-centered placement, collection-based discovery scoring, the historical discovery count, tab pause/resume, 10-second arrivals, and capacity game over. The canvas exposes `window.render_game_to_text()` and `window.advanceTime(ms)` for repeatable inspection; the latter switches that page instance to manual stepping until reload.
